...
float HFShapeRiverFlag(float2 requestedOffset)
{
	uint flags = HFShapeWaterFlags(requestedOffset);
	return (flags & HF_SHAPE_RIVER_MASK) != 0u &&
		(flags & HF_SHAPE_UNDERWATER_BIT) == 0u ? 1.0 : 0.0;
}
...
// The three edges incident to a corner belong to its three hex owners.
// Smooth degree-two chains across their shared node, instead of adding a
// separate wiggle to each edge. Sources and branching junctions stay anchored.
void HFRiverCornerFrame(int direction, int endpoint, float2 corner,
	float2 outgoing, uint ownerFlags, uint otherFlags,
	out float2 anchor, out float2 derivative)
{
	int ownerDirection = (direction + (endpoint == 0 ? 5 : 1)) % 6;
	int otherDirection = (direction + (endpoint == 0 ? 4 : 2)) % 6;
	bool ownerContinues = (ownerFlags & (1u << ownerDirection)) != 0u;
	bool otherContinues = (otherFlags & (1u << otherDirection)) != 0u;
	anchor = corner;
	derivative = outgoing;
	if (ownerContinues != otherContinues)
	{
		float2 alternateCenter = ownerContinues ?
			HFDirection(ownerDirection) * HF_SQRT3_OVER_2 :
			HFNeighborCenter(direction) + HFDirection(otherDirection) * HF_SQRT3_OVER_2;
		float2 alternate = 2.0 * (alternateCenter - corner);
		anchor += (outgoing + alternate) / 6.0;
		derivative = (outgoing - alternate) * 0.5;
	}
}

void HFRiverCurveParameters(
	float2 cellOffset, int direction,
	out float2 a, out float2 b, out float2 tangentA, out float2 tangentB)
{
	float2 localA, localB, unusedTangent;
	HFRiverEdgeSegment(direction, localA, localB, unusedTangent);
	uint ownerFlags = HFShapeWaterFlags(cellOffset);
	uint otherFlags = HFShapeWaterFlags(HFNeighborOffset(cellOffset, direction));
	float2 anchorA, anchorB, outgoingB;
	HFRiverCornerFrame(direction, 0, localA, localB - localA,
		ownerFlags, otherFlags, anchorA, tangentA);
	HFRiverCornerFrame(direction, 1, localB, localA - localB,
		ownerFlags, otherFlags, anchorB, outgoingB);
	tangentB = -outgoingB;
	float rowParity = fmod(cellOffset.y, 2.0);
	float2 cellCenter = float2(
		(cellOffset.x + rowParity * 0.5) * (2.0 * HF_SQRT3_OVER_2),
		cellOffset.y * 1.5);
	a = cellCenter + anchorA;
	b = cellCenter + anchorB;
	// Canonical endpoint order makes both owners of a shared edge produce the
	// same curve, normal, and texture coordinates. Sort the original corners;
	// a smoothed vertical edge may otherwise swap order after float rounding.
	if (localB.x < localA.x - 0.0001 ||
		(abs(localB.x - localA.x) <= 0.0001 && localB.y < localA.y))
	{
		float2 swap = a;
		a = b;
		b = swap;
...
void HFRiverCurveSample(
	float2 a, float2 b, float2 tangentA, float2 tangentB, float t,
	out float2 curvePosition, out float2 tangent)
{
	// Hermite form of a uniform cubic B-spline on degree-two chains. Relative
	// coordinates avoid cancellation at the large world map's far east edge.
	float2 chord = b - a;
	float2 quadratic = 3.0 * chord - 2.0 * tangentA - tangentB;
	float2 cubic = tangentA + tangentB - 2.0 * chord;
	curvePosition = a + t * (tangentA + t * (quadratic + t * cubic));
	tangent = tangentA + t * (2.0 * quadratic + 3.0 * t * cubic);
}

// A dry/dry HF river terminates at a corner shared with a third, wet cell.
// Continue only its rendering into that first wet cell, so the analytical
// channel meets HF's blended shoreline without adding gameplay river bits.
// Endpoints use the unsorted edge order; all positions use global outer radii.
bool HFRiverMouthSegment(float2 cellOffset, int direction, int endpoint,
	out float2 start, out float2 end)
{
	start = end = 0.0;
	uint flags = HFShapeWaterFlags(cellOffset);
	[branch] if ((flags & HF_SHAPE_COAST_BIT) == 0u ||
		(flags & HF_SHAPE_UNDERWATER_BIT) != 0u ||
		(flags & (1u << direction)) == 0u) return false;
	float2 resolved;
	float2 other = HFNeighborOffset(cellOffset, direction);
	if (!HFResolveOffset(other, resolved) ||
		(HFShapeWaterFlags(other) & HF_SHAPE_UNDERWATER_BIT) != 0u) return false;
	int thirdDirection = (direction + (endpoint == 0 ? 5 : 1)) % 6;
	float2 third = HFNeighborOffset(cellOffset, thirdDirection);
	if ((HFShapeWaterFlags(third) & HF_SHAPE_UNDERWATER_BIT) == 0u) return false;
	float2 localA, localB, unusedTangent;
	HFRiverEdgeSegment(direction, localA, localB, unusedTangent);
	float2 corner = endpoint == 0 ? localA : localB;
	float2 center = float2(
		(cellOffset.x + fmod(cellOffset.y, 2.0) * 0.5) * (2.0 * HF_SQRT3_OVER_2),
		cellOffset.y * 1.5);
	float2 towardWater = HFNeighborCenter(thirdDirection) - corner;
	start = center + corner;
	// A corner is one outer radius from the third cell's center. Keep the
	// extension inside that cell even if the style requests a longer plume.
	end = start + normalize(towardWater) * clamp(_HexRiverMouth.x, 0.05, 1.0);
	return true;
}
	...
	out float distance, out float2 delta, out float2 tangent, out float t)
{
	...
	HFRiverCurveParameters(
		cellOffset, direction, a, b, tangentA, tangentB);
	float2 chord = b - a;
	...
	[branch] if (HFShapeCoastFlag(cellOffset) > 0.5)
	{
		[unroll] for (int endpoint = 0; endpoint < 2; endpoint++)
		{
			float2 mouthStart, mouthEnd;
			if (!HFRiverMouthSegment(cellOffset, direction, endpoint, mouthStart, mouthEnd)) continue;
			float2 segment = mouthEnd - mouthStart;
			float along = saturate(dot(globalPoint - mouthStart, segment) /
				max(dot(segment, segment), 0.0001));
			float2 mouthDelta = globalPoint - (mouthStart + segment * along);
			float mouthDistance = length(mouthDelta);
			if (mouthDistance >= distance) continue;
			float endpointT = dot(mouthStart - a, chord) > 0.5 ? 1.0 : 0.0;
			float advance = endpointT > 0.5 ? 1.0 : -1.0;
			distance = mouthDistance;
			delta = mouthDelta;
			tangent = normalize(segment) * advance;
			t = endpointT + advance * along * length(segment);
		}
	}
}
...
void HFAccumulateDryRiverCoordinates(
	inout float riverDistance, inout float2 riverUV,
	float2 requestedOffset, float2 samplePoint)
{
	uint flags = HFShapeWaterFlags(requestedOffset);
	uint riverMask = flags & HF_SHAPE_RIVER_MASK;
	[branch] if (riverMask == 0u || (flags & HF_SHAPE_UNDERWATER_BIT) != 0u) return;
	float candidateDistance;
	float2 candidateUV;
	HFRiverCoordinates(samplePoint, requestedOffset, riverMask, candidateDistance, candidateUV);
...
// Pixel and height queries use the same continuous field. Unsigned vertex
// distances cannot be interpolated: a narrow curved core can cross a triangle
// whose three vertices all lie outside the channel.
void HF_EvaluateRiverCoordinates(float cellIndex, float2 localPosition,
	out float riverDistance, out float2 riverUV)
{
	float2 rootOffset = HFCellIndexToOffset(cellIndex);
	riverDistance = 1000.0;
	riverUV = 0.0;
	HFAccumulateDryRiverCoordinates(riverDistance, riverUV, rootOffset, localPosition);
	[loop] for (int direction = 0; direction < 6; direction++)
	{
		float2 first = HFNeighborOffset(rootOffset, direction);
		float2 center = HFNeighborCenter(direction);
		HFAccumulateDryRiverCoordinates(riverDistance, riverUV, first, localPosition - center);
		int next = (direction + 1) % 6;
		HFAccumulateDryRiverCoordinates(riverDistance, riverUV,
			HFNeighborOffset(first, next), localPosition - center - HFNeighborCenter(next));
	}
}

float HFRiverCoreCoverage(float riverDistance)
{
	return 1.0 - smoothstep(_HexReliefRiverCarve.x * 0.72,
		_HexReliefRiverCarve.x + 0.065, riverDistance);
}

float HFRiverSurfaceVisibility(float seaInfluence, float signedWaterDepth)
{
	// The sea mask alone does not guarantee ocean coverage on a steep bank.
	// Keep the channel visible until both the sea blend and its height contour
	// cover it, using the near ocean's minimum contour feather for overlap.
	float seaCoverage = smoothstep(0.08, 0.32, seaInfluence) *
		smoothstep(-0.06, 0.06, signedWaterDepth);
	return 1.0 - seaCoverage;
}
	...
	out float2 modulePoint, out float moduleIndex)
{
	...
	bool mountain = cell.landform > 1.5 && cell.landform < 2.5;
	...
	float2 samplePoint)
{
	...
	// River geometry is independent of a cell's randomly rotated material
	// stamp; especially its mouth must reach past the dry stamp into water.
	HFAccumulateDryRiverCoordinates(accumulator.riverDistance, accumulator.riverUV,
		requestedOffset, samplePoint);
...
HFReliefSurface HF_EvaluateLegacyRelief(float cellIndex, float2 localPosition)
{
	...
	float maximum = result.landform < 1.5 || result.landform > 2.5 ?
		_HexReliefHeights.x :
...
#include "HexNearTerrainShape.hlsl"

// The dry Near surface replaces HF height completely, but retains HF's exact
// river neighborhood and rotated stamp gate. Decode only the fields needed by
// the river calculation; no authored HF height or mixer samples are necessary.
void HFAccumulateNearDryRiver(float2 requestedOffset, float2 samplePoint,
	inout float riverDistance, inout float2 riverUV)
{
	HFAccumulateDryRiverCoordinates(riverDistance, riverUV, requestedOffset, samplePoint);
}

bool HFTryEvaluateNearDryRelief(float cellIndex, float2 localPosition,
	out HFReliefSurface result)
{
	result = (HFReliefSurface)0;
	float2 rootOffset = HFCellIndexToOffset(cellIndex);
	HFCellShape rootCell = HFLoadCell(rootOffset);
	[branch] if (rootCell.valid < 0.5) return false;
	// HexCellShaderData marks both owners and every cell within two graph
	// steps of a wet/dry transition. If these flags are clear, all thirteen HF
	// contributors are dry, so their weighted sea value is exactly zero.
	uint waterFlags = HFShapeWaterFlags(rootOffset);
	[branch] if ((waterFlags & (HF_SHAPE_UNDERWATER_BIT | HF_SHAPE_COAST_BIT)) != 0u) return false;
	float2 rootUV = HFOriginalUV(localPosition, rootCell.angle);
	// A positive root stamp guarantees positive HF totalWeight and coverage=1,
	// including explicit-root samples slightly outside the hex. Otherwise use
	// the complete reconstruction rather than assuming that it has coverage.
	[branch] if (HFOriginalCentralization(rootUV) <= 0.0001) return false;
	result.y = _HexHFOriginalDatumY + 0.018;
	result.bakedHeight = 0.5;
	result.coverage = 1.0;
	result.landform = rootCell.landform;
	result.terrain = rootCell.terrain;
	result.style = rootCell.hash;
	result.moduleUV = rootUV;
	result.moduleIndex = HFOriginalPanelForCell(rootCell.terrain,
		rootCell.landform, rootCell.plantLevel, rootCell.underwater);
	result.riverDistance = 1000.0;
	HFAccumulateNearDryRiver(rootOffset, localPosition, result.riverDistance, result.riverUV);
	[loop] for (int direction = 0; direction < 6; direction++)
	{
		float2 first = HFNeighborOffset(rootOffset, direction);
		float2 center = HFNeighborCenter(direction);
		HFAccumulateNearDryRiver(first, localPosition - center, result.riverDistance, result.riverUV);
		int next = (direction + 1) % 6;
		HFAccumulateNearDryRiver(HFNeighborOffset(first, next),
			localPosition - (center + HFNeighborCenter(next)), result.riverDistance, result.riverUV);
	}
	return true;
}
...
HFReliefSurface HF_EvaluateRelief(float cellIndex, float2 localPosition)
{
	...
	{
		HFReliefSurface surface;
		// Keep all legacy and coast paths intact. Strongest-stamp material
		// metadata is unused by Near's logical-biome fragment reconstruction.
		[branch] if (_HexNearTerrainEnabled <= 0.5)
			return HF_EvaluateOriginalRelief(cellIndex, localPosition);
		[branch] if (!HFTryEvaluateNearDryRelief(cellIndex, localPosition, surface))
			surface = HF_EvaluateOriginalRelief(cellIndex, localPosition);
		surface.y = NearEvaluateY(cellIndex, localPosition, surface.seaInfluence,
			surface.riverDistance, surface.y);
		surface.height = max(surface.y - _HexHFOriginalDatumY, 0.0);
		surface.height01 = saturate(surface.height / max(_HexNearHeights.x, 0.1));
		surface.bakedHeight = saturate(0.5 + surface.height / _HexHFOriginalHeightScale);
		return surface;
	}
