Shader "Hex Map/Relief"
{
	Properties
	{
        ...
        [NoScaleOffset][NonModifiableTextureData] _Civ6RiverWaveMoments ("Civ6 River Wave Moments", 2D) = "gray" {}
        [NoScaleOffset][NonModifiableTextureData] _Civ6RiverDensity ("Civ6 River Density", 2D) = "white" {}
        [NoScaleOffset][NonModifiableTextureData] _Civ6RiverScatter ("Civ6 River Scatter", 2D) = "gray" {}
        [NoScaleOffset][NonModifiableTextureData] _Civ6RiverBankAlbedo ("Civ6 River Bank Albedo", 2D) = "gray" {}
        _Civ6RiverWorldScale ("River Ripple World Scale", Range(.01, 1)) = 0.18
        _Civ6RiverScrollSpeed ("River Ripple Scroll Speed", Range(0, .2)) = 0.0375
        _Civ6RiverBumpStrength ("River Ripple Amplitude", Range(0, 6)) = 1.75
        _Civ6RiverOpticalDepth ("River Visual Depth", Range(.1, 3)) = 1.2
        _Civ6RiverDensityRange ("River Density Lookup Range", Range(.1, 5)) = 2.0
        _Civ6RiverDensityStrength ("River Density Strength", Range(0, 3)) = 1.0
        _Civ6RiverScatterTint ("River Source Scatter Tint", Range(0, 1)) = 0.18
        _Civ6RiverBankTextureStrength ("River Bank Texture Strength", Range(0, 1)) = 0.32
        _Civ6RiverWaterDarkening ("River Core Color Scale", Range(.1, 1)) = 0.68
        _Civ6RiverF0 ("River Fresnel F0", Range(.0001, .04)) = 0.004
        _Civ6RiverSpecularExponent ("River Sun Exponent", Range(32, 5000)) = 850
        _Civ6RiverSunStrength ("River Sun Strength", Range(0, 4)) = 1.2
        _Civ6RiverSkyStrength ("River Sky Reflection", Range(0, 1)) = 0.28
	...
	SubShader
	{
		...
		Pass
		{
			...
			#include "HexReliefTessellation.hlsl"
			#include "HexNearLandMaterial.hlsl"
			...
			float ReliefTessellationFactor(float3 positionOS)
			{
				...
				float factor = max(1.0, lerp(_HexReliefTessellation, 1.0, distanceFade));
				if (_HexNearTerrainEnabled > .5)
				{
					float nearFade = saturate((distanceToCamera - _HexNearTessellation.y) /
						max(_HexNearTessellation.z - _HexNearTessellation.y, 1.0));
					factor = max(factor, lerp(_HexNearTessellation.x, 1.0, nearFade));
				}
				return factor;
			}

			float CoastTessellationScale(float3 positionOS)
			{
				// 远距时海岸强制细分也衰减，避免中近景仍按 coastMin=8 细分。
				float factor = ReliefTessellationFactor(positionOS);
				return saturate(
					(factor - 1.0) / max(_HexReliefTessellation - 1.0, 1.0));
			}
				...
				InputPatch<TessControlPoint, 3> patch)
			{
				...
				float3 patchCenterOS =
					(patch[0].positionOS.xyz + patch[1].positionOS.xyz +
					patch[2].positionOS.xyz) / 3.0;
				float coastScale = CoastTessellationScale(patchCenterOS);
				float rootMinimum = lerp(
					1.0, _HexReliefCoastTessellation, rootCoast * coastScale);
				float outerMinimum = lerp(
					1.0, _HexReliefCoastTessellation,
					max(rootCoast, neighborCoast) * coastScale);
				// Retain the carved bed on river cells when ordinary distance
				// tessellation fades. Both owners agree on their shared edge.
				float rootRiver = HFShapeRiverFlag(rootOffset);
				float neighborRiver = HFShapeRiverFlag(HFNeighborOffset(rootOffset, outerDirection));
				rootMinimum = max(rootMinimum, lerp(1.0, 6.0, rootRiver));
				outerMinimum = max(outerMinimum, lerp(1.0, 6.0, max(rootRiver, neighborRiver)));
				float2 nearMinimum = HFNearReliefPatchMinimum(rootOffset, outerDirection);
				rootMinimum = max(rootMinimum, nearMinimum.x);
				outerMinimum = max(outerMinimum, nearMinimum.y);
				...
				factors.inside = max(max(rootMinimum, nearMinimum.y), ReliefTessellationFactor(
					patchCenterOS));
				return factors;
			}
			...
			half4 Frag(Varyings input) : SV_Target
			{
				...
				{
					// The patch's input normals are flat; recover the displaced surface
					// normal so new height fields really receive directional lighting.
					half3 geometricNormal = normalize(cross(ddy(input.positionWS), ddx(input.positionWS)));
					vertexNormal = geometricNormal.y < 0.0h ? -geometricNormal : geometricNormal;
				}
				...
				float isMountain = step(1.5, landform) * (1.0 - step(2.5, landform));
				...
				{
					// Reconstruct the narrow curve per pixel. Interpolated unsigned
					// vertex distances can erase its core inside a coarse triangle.
					HF_EvaluateRiverCoordinates(input.cellIndices.x, input.localPosition,
						input.waterData.z, input.riverUV);
					half3 color;
					HexNearLandMaterial nearLand = (HexNearLandMaterial)0;
					if (_HexNearTerrainEnabled > 0.5)
					{
						nearLand = EvaluateHexNearLandMaterial(input.cellIndices.x, input.localPosition, input.positionWS, vertexNormal);
						color = nearLand.albedo;
						// Keep the true face normal for rock and snow placement above.
						// Gentle ground needs softer lighting than exposed cliff faces;
						// otherwise side light emphasizes every tessellation triangle.
						half3 shapeNormal = vertexNormal;
						float gentleGround = smoothstep(.55, .94, shapeNormal.y) *
							smoothstep(.20, .38, input.waterData.z) *
							(1.0 - smoothstep(.03, .22, input.waterData.x));
						half3 detailNormal = nearLand.normalWS;
						vertexNormal = normalize(lerp(shapeNormal, detailNormal, lerp(1.0, .55, gentleGround)));
						vertexNormal = normalize(lerp(vertexNormal, normalize(input.normalWS), .36 * gentleGround));
					}
					else color = HF_EvaluateOriginalDiffuse(input.cellIndices.x, input.localPosition);
					half riverSurfaceMask = 0.0h;
					HexRiverSurface riverSurface = (HexRiverSurface)0;
					...
					half3 deepFloor = lerp(
						_HexDeepOceanColor.rgb * 1.05h,
						_HexDeepOceanColor.rgb * 0.82h,
						smoothstep(0.55h, 3.2h, waterDepth));
					deepFloor = lerp(
						_HexShallowWaterColor.rgb * half3(0.55h, 0.75h, 0.82h),
						deepFloor,
						smoothstep(0.15h, 1.10h, waterDepth));
					half3 coastArt = lerp(
						sand, deepFloor, smoothstep(0.18h, 0.95h, waterDepth));
					half hfDetail = dot(
						color, half3(0.299h, 0.587h, 0.114h));
					coastArt *= lerp(0.88h, 1.12h, hfDetail);
					color = lerp(
						color, coastArt,
						smoothstep(0.04h, 0.72h, seaInfluence) * submerged);
					if (_HexNearTerrainEnabled > .5)
					{
						float beach = smoothstep(.025, .17, seaInfluence) *
							(1.0 - smoothstep(.20, .65, input.positionWS.y - input.waterData.y));
						color = lerp(color, sand, beach * (1.0 - submerged) * .85);
					}

					// Preserve the intersection-free analytical channel, then shade its
					// core as water. Static river distance owns the optical cross-section;
					// ripples use a continuous global phase, without claiming downstream flow.
					float riverBank = 1.0 - smoothstep(
						_HexReliefRiverCarve.x + 0.015,
						_HexReliefRiverCarve.x + 0.12,
						input.waterData.z);
					float riverCore = HFRiverCoreCoverage(input.waterData.z);
					// Fade with the continuous water blend, overlapping the ocean
					// layer instead of disappearing at the first wet stamp sample.
						half visibleRiver = HFRiverSurfaceVisibility(seaInfluence,
							_HexHFOriginalDatumY - input.positionWS.y);
					riverSurfaceMask = (half)riverCore * visibleRiver;
					float3 positionDx = ddx(input.positionWS);
					float3 positionDy = ddy(input.positionWS);
					color = lerp(
						color, EvaluateHexRiverBankColor(input.positionWS.xz,
							positionDx.xz, positionDy.xz, riverBank * visibleRiver),
						riverBank * 0.56h * visibleRiver);

					// All texture gradients were captured outside the narrow water
					// branch. Hashed edge UVs never drive normal or animation phase.
					[branch]
					if (riverSurfaceMask > 0.001h)
					{
						riverSurface = EvaluateHexRiverSurface(input.positionWS.xz,
							positionDx.xz, positionDy.xz, input.waterData.z);
						vertexNormal = normalize(lerp(
							vertexNormal, riverSurface.normalWS, riverSurfaceMask));
						color = lerp(color, riverSurface.baseColor,
					...
					if (_HexNearTerrainEnabled > 0.5 && submerged < 0.5)
					{
						// Albedo-only near assets need a directional light response.
						// The legacy 30% diffuse floor plus full SH flattened opposing
						// mountain faces, especially after the old 600-unit shadow cut.
						// Retain cool ambient fill so valleys still reveal their material.
						half3 ambient = SampleSH(vertexNormal) * 0.62h +
							half3(0.065h, 0.078h, 0.095h);
						lighting = ambient + mainLight.color *
							(0.06h + 0.94h * rawDiffuse) * shadowAttenuation;
					}
					color *= min(lighting, 1.2h);
					// A weak material response uses the source gloss only on visible
					// dry terrain. The existing analytical river/water lighting follows.
					[branch]
					if (_HexNearTerrainEnabled > .5 && submerged < .5)
					{
						float3 landView = SafeNormalize(_WorldSpaceCameraPos.xyz - input.positionWS);
						color += HexNearLandDirectSpecular(nearLand, vertexNormal, landView,
							mainLight.direction, mainLight.color) * shadowAttenuation *
							(1.0h - riverSurfaceMask) * lerp(.25h, 1.0h, cellData.r);
					}


					// Filtered source-height moments keep a narrow moving glint
					// without the old broad white/blue river reflection wash.
					[branch]
					if (riverSurfaceMask > 0.001h)
					{
						half3 viewDirection = SafeNormalize(
							_WorldSpaceCameraPos.xyz - input.positionWS);
						half3 waterLight = EvaluateHexRiverReflection(riverSurface,
							riverSurface.normalWS, viewDirection, mainLight.direction,
		...
		Pass
		{
			...
			float ShadowCoastTessellationScale(float3 positionOS)
			{
				float factor = ShadowTessellationFactor(positionOS);
				return saturate(
					(factor - 1.0) / max(_HexReliefTessellation - 1.0, 1.0));
			}
				...
				InputPatch<ShadowControlPoint, 3> patch)
			{
				...
				float3 patchCenterOS =
					(patch[0].positionOS.xyz + patch[1].positionOS.xyz +
					patch[2].positionOS.xyz) / 3.0;
				float coastScale = ShadowCoastTessellationScale(patchCenterOS);
				float rootMinimum = lerp(
					1.0, _HexReliefCoastTessellation, rootCoast * coastScale);
				float outerMinimum = lerp(
					1.0, _HexReliefCoastTessellation,
				...
				factors.edge[2] = max(rootMinimum, ShadowTessellationFactor(
					(patch[0].positionOS.xyz + patch[1].positionOS.xyz) * 0.5));
				factors.inside = max(max(rootMinimum, nearMinimum.y), ShadowTessellationFactor(
					patchCenterOS));
				return factors;
			}
				...
				float3 barycentricCoordinates : SV_DomainLocation)
			{
				...
				float isMountain = step(1.5, surface.landform) * (1.0 - step(2.5, surface.landform));
