...
#pragma warning restore IDE0044 // Add readonly modifier
	...
	public readonly HexMountainMode MountainMode => grid.CellData[index].mountainMode;

	public readonly void SetMountainMode(HexMountainMode mode)
	{
		mode = (HexMountainMode)Mathf.Clamp((int)mode, 0, 2);
		if (grid.CellData[index].mountainMode == mode) return;
		grid.CellData[index].mountainMode = mode;
		grid.RefreshCellWithDependents(index);
	}
