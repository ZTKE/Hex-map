...
public class HexMapGenerator : MonoBehaviour
{
	...
	public bool GenerateMapFromLandMask(
		...
		float sourceVMin = 0f,
		float sourceVMax = 1f,
		HexWorldTerrainAtlas terrainAtlas = null)
	{
		...
		if (float.IsNaN(sourceVMin) || float.IsNaN(sourceVMax) ||
			sourceVMin < 0f || sourceVMax > 1f ||
		...
		if (terrainAtlas != null && !terrainAtlas.ValidateGeometry(
			x, z, sourceVMin, sourceVMax, out string atlasError))
		{
			Debug.LogError(atlasError);
			return false;
		}
		...
		try
		{
			...
			else if (!terrainAtlas.ApplyToGrid(
				grid, sourceVMin, sourceVMax, out string applyError, refresh: false))
			{
				Debug.LogError(applyError);
				return false;
			}
			grid.RefreshAllCells();
		}
