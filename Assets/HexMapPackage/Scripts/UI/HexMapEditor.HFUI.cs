...
public partial class HexMapEditor
{
	...
	{
		"Flat", "Hill", "Mountain", "Plateau"
	};

	static readonly string[] hfVegetationNames =
	{
		"Natural Mix", "Broadleaf", "Sapling", "Conifer", "Deadwood", "Cold Mix", "Jungle"
	};
	...
	static readonly string[] mountainModeNames = { "Automatic", "Continuous Range", "Mountain Massif" };
	...
	void OnGUI()
	{
		...
		if (activeTool == EditorTool.Relief && activeLandform == HexLandform.Mountain)
			activeMountainMode = (HexMountainMode)GUILayout.Toolbar(
				(int)activeMountainMode, mountainModeNames, GUILayout.Height(22f));
	...
	string GetActiveToolHint() => activeTool switch
	{
		...
		EditorTool.Relief =>
			activeLandform == HexLandform.Mountain ?
				"Range joins high crests; Massif keeps valleys. Automatic follows the local mountain group." :
				"Relief paints Flat / Hill / Mountain / Plateau without changing the terrain material.",
		EditorTool.Forest =>
	...
	string GetHoverDescription()
	{
		...
		return $"Hover {data.coordinates}  |  {hfTerrainNames[terrain]}  |  " +
			$"R{data.TerrainRotation * 60} deg  |  {data.landform}" +
			(data.landform == HexLandform.Mountain ? $" ({data.mountainMode})" : "") + $"  |  {forest}  |  " +
	...
	void PickHoveredToolSettings()
	{
		...
		switch (activeTool)
		{
			...
			case EditorTool.Relief:
				...
				activeMountainMode = data.mountainMode;
				break;
	...
	Rect GetHFPanelRect()
	{
		...
		float height = activeTool == EditorTool.Forest ? hfForestPanelHeight :
			activeTool == EditorTool.Terrain || (activeTool == EditorTool.Relief && activeLandform == HexLandform.Mountain) ?
				hfTerrainPanelHeight : hfPanelHeight;
		return new Rect(
