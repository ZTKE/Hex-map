...
public sealed class HexSurfaceSampler
{
	...
	public HexTerrainStyle Style => style;
		...
		int rootCellIndex, Vector3 localPosition, bool carveRiver = true)
	{
		...
		// Match the shader's coast-bit proof: every possible HF contributor is
		// dry, so sea is exactly zero and Near completely replaces the old Y.
		// The root gate also proves that HF coverage would be one.
		if (style.UsesNearTerrain && IsDryTwoRingInterior(rootX, rootZ) &&
			Centralization(OriginalStampUV(rootCellIndex, rootPoint)) > 0.0001f)
		{
			float riverDistance = 1000f;
			if (carveRiver)
			{
				AccumulateDryRiver(rootX, rootZ, rootPoint, ref riverDistance);
				for (int direction = 0; direction < 6; direction++)
				{
					Vector2Int first = NeighborOffset(rootX, rootZ, direction);
					Vector2 firstCenter = neighborCenters[direction];
					AccumulateDryRiver(first.x, first.y, rootPoint - firstCenter, ref riverDistance);
					int next = (direction + 1) % 6;
					Vector2Int corner = NeighborOffset(first.x, first.y, next);
					AccumulateDryRiver(corner.x, corner.y,
						rootPoint - firstCenter - neighborCenters[next], ref riverDistance);
				}
			}
			return HexNearTerrainSurface.Evaluate(grid, style.nearTerrainProfile,
				rootX, rootZ, rootPoint, 0f, riverDistance, DatumY + hfSurfaceBias, carveRiver);
		}
		...
		float surfaceY = DatumY + displacement + hfSurfaceBias;
		if (style.UsesNearTerrain)
			return HexNearTerrainSurface.Evaluate(grid, style.nearTerrainProfile,
				rootX, rootZ, rootPoint, seaInfluence, accumulator.riverDistance, surfaceY, carveRiver);
		return surfaceY;
	}

	float DatumY => HexMetrics.visualWaterLevel * HexMetrics.elevationStep;

	bool IsDryTwoRingInterior(int rootX, int rootZ)
	{
		if (IsWet(rootX, rootZ)) return false;
		for (int direction = 0; direction < 6; direction++)
		{
			Vector2Int first = NeighborOffset(rootX, rootZ, direction);
			Vector2Int corner = NeighborOffset(first.x, first.y, (direction + 1) % 6);
			Vector2Int axial = NeighborOffset(first.x, first.y, direction);
			if (IsWet(first.x, first.y) || IsWet(corner.x, corner.y) || IsWet(axial.x, axial.y))
				return false;
		}
		return true;
	}

	bool IsWet(int requestedX, int requestedZ) =>
		ResolveOffset(requestedX, requestedZ, out int x, out int z) &&
		grid.CellData[x + z * grid.CellCountX].IsUnderwater;

	Vector2 OriginalStampUV(int cellIndex, Vector2 samplePoint)
	{
		...
		return oriented / (2f * Mathf.Max(style.hfOriginalStampScale, 0.1f)) +
			new Vector2(0.5f, 0.5f);
	}

	void AccumulateDryRiver(int requestedX, int requestedZ, Vector2 samplePoint,
		ref float riverDistance)
	{
		if (!ResolveOffset(requestedX, requestedZ, out int x, out int z)) return;
		int cellIndex = x + z * grid.CellCountX;
		HexCellData cell = grid.CellData[cellIndex];
		if (!cell.HasRiver || cell.IsUnderwater) return;
		RiverCoordinates(samplePoint, requestedX, requestedZ, cell, out float candidateDistance, out _);
		if (candidateDistance < riverDistance) riverDistance = candidateDistance;
	}
		...
		int requestedX, int requestedZ, Vector2 samplePoint, bool carveRiver)
	{
		...
		int cellIndex = x + z * grid.CellCountX;
		HexCellData cell = grid.CellData[cellIndex];
		// Geometry is independent of the randomly rotated material stamp. A
		// coastal corner's visual continuation must also reach its wet neighbor.
		if (carveRiver && !cell.IsUnderwater && cell.HasRiver)
		{
			RiverCoordinates(samplePoint, requestedX, requestedZ, cell,
		...
		Vector2 uv = OriginalStampUV(cellIndex, samplePoint);
	...
	// Matches HFRiverMouthSegment: unsorted edge endpoints and global outer
	// radii. This extends rendering into one wet cell without changing the map.
	bool TryRiverMouthSegment(int cellX, int cellZ, int direction, int endpoint,
		out Vector2 start, out Vector2 end)
	{
		start = end = Vector2.zero;
		if (!ResolveOffset(cellX, cellZ, out int x, out int z)) return false;
		HexCellData cell = grid.CellData[x + z * grid.CellCountX];
		if (cell.IsUnderwater || !cell.HasRiverThroughEdge((HexDirection)direction)) return false;
		Vector2Int other = NeighborOffset(cellX, cellZ, direction);
		if (!ResolveOffset(other.x, other.y, out int otherX, out int otherZ) ||
			grid.CellData[otherX + otherZ * grid.CellCountX].IsUnderwater) return false;
		int thirdDirection = (direction + (endpoint == 0 ? 5 : 1)) % 6;
		Vector2Int third = NeighborOffset(cellX, cellZ, thirdDirection);
		if (!IsWet(third.x, third.y)) return false;
		RiverEdgeSegment(direction, out Vector2 localA, out Vector2 localB, out _);
		Vector2 corner = endpoint == 0 ? localA : localB;
		Vector2 center = new(
			(cellX + (cellZ & 1) * 0.5f) * (2f * sqrt3Over2), cellZ * 1.5f);
		Vector2 towardWater = neighborCenters[thirdDirection] - corner;
		start = center + corner;
		end = start + towardWater.normalized * Mathf.Clamp(style.riverMouthLength, 0.05f, 1f);
		return true;
	}
		...
		out Vector2 tangent, out float t)
	{
		RiverCurveParameters(
			...
			out Vector2 tangentA, out Vector2 tangentB);
		Vector2 chord = b - a;
		t = Mathf.Clamp01(
		...
		{
			if (!TryRiverMouthSegment(cellX, cellZ, direction, endpoint,
				out Vector2 mouthStart, out Vector2 mouthEnd)) continue;
			Vector2 segment = mouthEnd - mouthStart;
			float along = Mathf.Clamp01(Vector2.Dot(globalPoint - mouthStart, segment) /
				Mathf.Max(Vector2.Dot(segment, segment), 0.0001f));
			Vector2 mouthDelta = globalPoint - (mouthStart + segment * along);
			float mouthDistance = mouthDelta.magnitude;
			if (mouthDistance >= distance) continue;
			float endpointT = Vector2.Dot(mouthStart - a, chord) > 0.5f ? 1f : 0f;
			float advance = endpointT > 0.5f ? 1f : -1f;
			distance = mouthDistance;
			delta = mouthDelta;
			tangent = segment.normalized * advance;
			t = endpointT + advance * along * segment.magnitude;
		}
	}
		...
		out Vector2 tangentA, out Vector2 tangentB)
	{
		...
		Vector2 center = new(
			(cellX + (cellZ & 1) * 0.5f) * (2f * sqrt3Over2),
			cellZ * 1.5f);
		byte ownerRivers = ResolveOffset(cellX, cellZ, out int x, out int z) ?
			RiverMask(grid.CellData[x + z * grid.CellCountX]) : (byte)0;
		Vector2Int other = NeighborOffset(cellX, cellZ, direction);
		byte otherRivers = ResolveOffset(other.x, other.y, out int otherX, out int otherZ) ?
			RiverMask(grid.CellData[otherX + otherZ * grid.CellCountX]) : (byte)0;
		RiverCornerFrame(direction, 0, ownerRivers, otherRivers, localA, localB - localA,
			out Vector2 anchorA, out tangentA);
		RiverCornerFrame(direction, 1, ownerRivers, otherRivers, localB, localA - localB,
			out Vector2 anchorB, out Vector2 outgoingB);
		a = center + anchorA;
		b = center + anchorB;
		tangentB = -outgoingB;
		// Use the original edge order so moving a shared anchor cannot change
		// the orientation seen from its reciprocal cell.
		if (localB.x < localA.x - 0.0001f ||
			(Mathf.Abs(localB.x - localA.x) <= 0.0001f && localB.y < localA.y))
		{
			(a, b) = (b, a);
			(tangentA, tangentB) = (-tangentB, -tangentA);
		}
	}

	static byte RiverMask(HexCellData cell)
	{
		// HexFlags stores the six incoming bits at 6 and outgoing bits at 12.
		// Match the combined HasRiverThroughEdge mask sent to the shader.
		int flags = (int)cell.flags;
		return (byte)((cell.hfRiverEdges | (flags >> 6) | (flags >> 12)) & 0b111111);
	}

	static void RiverCornerFrame(
		int direction, int endpoint, byte ownerRivers, byte otherRivers,
		Vector2 corner, Vector2 outgoing,
		out Vector2 anchor, out Vector2 derivative)
	{
		anchor = corner;
		derivative = outgoing;
		int ownerDirection = (direction + (endpoint == 0 ? 5 : 1)) % 6;
		int otherDirection = (direction + (endpoint == 0 ? 4 : 2)) % 6;
		bool ownerRiver = (ownerRivers & (1 << ownerDirection)) != 0;
		bool otherRiver = (otherRivers & (1 << otherDirection)) != 0;
		// Only degree-two corners move. Sources, mouths and three-way junctions
		// keep their common authored corner.
		if (ownerRiver == otherRiver) return;
		Vector2 alternativeCenter = ownerRiver ?
			riverDirections[ownerDirection] * sqrt3Over2 :
			neighborCenters[direction] + riverDirections[otherDirection] * sqrt3Over2;
		Vector2 alternativeOutgoing = (alternativeCenter - corner) * 2f;
		anchor = corner + (outgoing + alternativeOutgoing) / 6f;
		derivative = (outgoing - alternativeOutgoing) * 0.5f;
	}

	static void RiverCurveSample(
		Vector2 a, Vector2 b, Vector2 tangentA, Vector2 tangentB, float t,
		out Vector2 point, out Vector2 tangent)
	{
		Vector2 chord = b - a;
		Vector2 quadratic = chord * 3f - tangentA * 2f - tangentB;
		Vector2 cubic = tangentA + tangentB - chord * 2f;
		point = a + (tangentA + (quadratic + cubic * t) * t) * t;
		tangent = tangentA + (quadratic * 2f + cubic * (3f * t)) * t;
	}
