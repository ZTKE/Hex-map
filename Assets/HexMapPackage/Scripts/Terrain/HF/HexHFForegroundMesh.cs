...
public sealed class HexHFForegroundMesh : MonoBehaviour
{
	...
	void Awake()
	{
		mesh = new Mesh
		{
			name = "HF Foreground Chunk Batch",
	...
	public void Configure(HexTerrainStyle style) =>
	...
	/// <summary>Return an interrupted chunk build's temporary buffers to their pools.</summary>
	public void CancelConstruction() => ReleaseLists();
	...
	public void AddCell(HexCellData cell, int cellIndex, Vector3 center)
	{
		...
		HexHFForegroundProfile profile =
			(terrainStyle ? terrainStyle : HexTerrainStyle.RuntimeDefault)
			.GetForegroundProfile(cell.vegetation);
		if (profile == null || profile.entries == null)
		{
			return;
		}

		foreach (HexHFForegroundEntry entry in profile.entries)
		{
			AddGroup(
				entry.sprite, ScaleForestCount(entry.denseCount, density),
				ApplyVegetationTint(entry.baseTint, cell.vegetationTint),
	...
	{
		denseCount = Mathf.Clamp(denseCount, 0, 512);
		return denseCount == 0 ? 0 :
	...
	void AddGroup(
		HexHFForegroundSprite kind, int count, Color baseTint,
	...
	void AddSprite(
		HexHFForegroundSprite kind, Vector3 center, Vector2 localPosition,
	...
	static AtlasEntry GetEntry(HexHFForegroundSprite kind) => kind switch
	{
		HexHFForegroundSprite.Tree14 => new AtlasEntry(
			new Rect(0f, 0.75f, 0.5f, 0.25f), 128, 64, 88f, 58f),
		HexHFForegroundSprite.DeadTree07 => new AtlasEntry(
			new Rect(0.5078125f, 0.75f, 0.25f, 0.25f), 64, 64, 48f, 58f),
		HexHFForegroundSprite.Tree04 => new AtlasEntry(
			new Rect(0f, 0.1015625f, 0.25f, 0.125f), 64, 32, 41f, 28f),
		HexHFForegroundSprite.DeadTree02 => new AtlasEntry(
