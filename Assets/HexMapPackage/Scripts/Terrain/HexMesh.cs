...
public class HexMesh : MonoBehaviour
{
	...
	// Subdivision deliberately preserves triangle-local UVs and cell weights,
	// so many resulting vertices share a surface sample. Reuse only exact
	// positions with the same logical root, for this single conformance pass.
	readonly Dictionary<(int cellIndex, Vector3 position), float> surfaceHeights = new();
	...
	void OnDestroy()
	{
		...
		// Awake creates this mesh for the renderer. Returning a chunk to its
		// pool keeps it alive; destroying a map must release the native mesh too.
		if (hexMesh)
		{
			if (Application.isPlaying) Destroy(hexMesh);
			else DestroyImmediate(hexMesh);
			hexMesh = null;
		}
	}
	...
	/// </summary>
	public void Apply(bool useFlatUpNormals = false)
	{
	...
	/// <summary>Abort unfinished CPU work while retaining the published mesh.</summary>
	public void CancelConstruction() => ReleaseConstructionLists();
		...
		HexGrid grid, int subdivisionLevels, float verticalOffset)
	{
		...
		for (int i = 0; i < vertices.Count; i++)
		{
			...
			var key = (rootCellIndex, position);
			bool finitePosition = !float.IsNaN(position.x) && !float.IsInfinity(position.x) &&
				!float.IsNaN(position.y) && !float.IsInfinity(position.y) &&
				!float.IsNaN(position.z) && !float.IsInfinity(position.z);
			float height;
			if (!finitePosition || !surfaceHeights.TryGetValue(key, out height))
			{
				height = rootCellIndex >= 0 ?
					grid.SampleSurfaceHeight(rootCellIndex, position) :
					grid.SampleSurfaceHeight(position);
				if (finitePosition) surfaceHeights.Add(key, height);
			}
