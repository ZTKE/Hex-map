...
public sealed class HexTerrainStyle : ScriptableObject
{
	...
	[Tooltip("Optional near-view shape, material and vegetation set. Empty retains HF presentation.")]
	public HexNearTerrainProfile nearTerrainProfile;
	public bool UsesNearTerrain => UsesHFOriginalSurface && nearTerrainProfile && nearTerrainProfile.IsReady;
	...
	public Color deepOcean = new(0.018f, 0.10f, 0.20f, 1f);
	public Color shallowWater = new(0.06f, 0.36f, 0.44f, 1f);
	public Color shoreFoam = new(0.90f, 0.96f, 0.94f, 1f);
	...
	[Header("HoneyFramework foreground definitions")]
	[Tooltip("Tree recipes selected by the Forest brush: Mixed, Broadleaf, Sapling, Conifer, Deadwood, Cold Mixed.")]
	[SerializeField] HexHFForegroundProfile[] foregroundProfiles =
		BuildDefaultForegroundProfiles();
	...
	public float GetMaximumHeight(HexLandform landform, int terrainTypeIndex) =>
		landform == HexLandform.Plateau ?
			(UsesNearTerrain ? nearTerrainProfile.plateauHeight + nearTerrainProfile.plateauRelief : hillHeight) :
	...
	/// <summary>
	/// Get the data-driven HF tree recipe selected by a cell's Forest value.
	/// </summary>
	public HexHFForegroundProfile GetForegroundProfile(HexVegetation vegetation)
	{
		EnsureForegroundProfiles();
		// The legacy HF atlas has six recipes. Tropical cells use its broadleaf
		// fallback when the dedicated near-view jungle meshes are unavailable.
		if (vegetation == HexVegetation.Jungle)
		{
			vegetation = HexVegetation.Broadleaf;
		}
		return foregroundProfiles[Mathf.Clamp(
			(int)vegetation, 0, foregroundProfiles.Length - 1)];
	}

	/// <summary>
	/// Restore the tree recipes that previously lived as constants in
	/// HexHFForegroundMesh.
	/// </summary>
	public void ResetForegroundProfilesToDefaults()
	{
		foregroundProfiles = BuildDefaultForegroundProfiles();
		RefreshRuntimeState();
	}
	...
	{
		if (UsesNearTerrain) nearTerrainProfile.ApplyGlobals();
		else Shader.SetGlobalFloat("_HexNearTerrainEnabled", 0f);
	...
	void OnValidate() => RefreshRuntimeState();
	...
	void EnsureForegroundProfiles()
	{
		const int profileCount = 6;
		HexHFForegroundProfile[] defaults = null;
		if (foregroundProfiles == null || foregroundProfiles.Length != profileCount)
		{
			HexHFForegroundProfile[] previous = foregroundProfiles;
			defaults = BuildDefaultForegroundProfiles();
			if (previous != null)
			{
				for (int i = 0; i < Mathf.Min(previous.Length, defaults.Length); i++)
				{
					if (previous[i] != null)
					{
						defaults[i] = previous[i];
					}
				}
			}
			foregroundProfiles = defaults;
		}

		for (int i = 0; i < foregroundProfiles.Length; i++)
		{
			if (foregroundProfiles[i] == null)
			{
				defaults ??= BuildDefaultForegroundProfiles();
				foregroundProfiles[i] = defaults[i];
			}
			else if (foregroundProfiles[i].entries == null)
			{
				foregroundProfiles[i].entries = Array.Empty<HexHFForegroundEntry>();
			}
		}
	}
	...
	static HexHFForegroundProfile[] BuildDefaultForegroundProfiles() => new[]
	{
		new HexHFForegroundProfile("Natural Mix",
			Foreground(HexHFForegroundSprite.Tree14, 15, 0x71, 0x79, 0x2f),
			Foreground(HexHFForegroundSprite.Tree04, 30, 0x71, 0x79, 0x2f),
			Foreground(HexHFForegroundSprite.Tree07, 40, 0x65, 0x78, 0x35)),
		new HexHFForegroundProfile("Broadleaf",
			Foreground(HexHFForegroundSprite.Tree14, 68, 0x76, 0x83, 0x36)),
		new HexHFForegroundProfile("Sapling",
			Foreground(HexHFForegroundSprite.Tree04, 82, 0x79, 0x86, 0x38)),
		new HexHFForegroundProfile("Conifer",
			Foreground(HexHFForegroundSprite.Tree07, 72, 0x4f, 0x70, 0x38)),
		new HexHFForegroundProfile("Deadwood",
			Foreground(HexHFForegroundSprite.DeadTree02, 38, 0x78, 0x6b, 0x4d),
			Foreground(HexHFForegroundSprite.DeadTree07, 34, 0x6d, 0x62, 0x49)),
		new HexHFForegroundProfile("Cold Mix",
			Foreground(HexHFForegroundSprite.Tree07, 34, 0x3d, 0x5c, 0x3e),
			Foreground(HexHFForegroundSprite.DeadTree02, 36, 0x68, 0x70, 0x5a))
	};

	static HexHFForegroundEntry Foreground(
		HexHFForegroundSprite sprite, int count, byte r, byte g, byte b) =>
		new(sprite, count, new Color(r / 255f, g / 255f, b / 255f, 1f));
}
